...
from yang.sim.env import Env, RunStage
from yang.sim.ext import load_shared_lib
...
def run_worker_process(
    ...
    for lib in config.get("libs", []):
        load_shared_lib(lib)
    for name, op in config.get("py_operations", {}).items():
        op_cls = import_attr(op)
        register_operation(name, op_cls)

    env = Env(config, config_path=config_path)
...
class SubprocessWorker(object):
    ...
    def start(
        ...
        self.proc = Process(
            ...
            args=(self.name, conn2, config, run_options, mod_deps, config_path),
            name=self.name,
        )
        self.proc.start()

from data import DateIndex, Array
...
def compute_pnl_stats(
    ...
    if isinstance(ret, Array):
        ret = ret.data
    stats = sim.ext.compute_pnl_stats(sig, ret, univ_size, book_size, hedge_ii)
...
def compute_yearly_pnl_stats(
    ...
    if isinstance(ret, Array):
        ret = ret.data
    base_stats = sim.ext.compute_pnl_stats(sig, ret, univ_size, book_size, hedge_ii)
    stats = sim.ext.compute_yearly_pnl_stats(base_stats, dates, book_size, include_total)
...
def compute_tpnl_stats(
    ...
    if isinstance(prc, Array):
        prc = prc.data
    stats = sim.ext.compute_tpnl_stats(
...
def compute_yearly_tpnl_stats(
    ...
    if isinstance(prc, Array):
        prc = prc.data
    base_stats = sim.ext.compute_tpnl_stats(
        sig, ret, prc, univ_size, book_size, buy_fee, sell_fee, hedge_ii
    )
    stats = sim.ext.compute_yearly_pnl_stats(base_stats, dates, book_size, include_total)
    return format_yearly_stats(stats, dates)
